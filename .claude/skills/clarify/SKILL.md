...
2. Identify the underspecified areas: scope boundaries, edge cases, failure modes, success and acceptance criteria, and any invariant in CLAUDE.md the work might touch (its "Read first: safety and intent" and "Do not" sections).
...
4. Record the answers. If the target is a spec file, add or update a "## Clarifications" section with the questions and answers, and append the current date to the spec's `Modified` list (if the spec predates the metadata block, add it first: adopt-on-touch). If it is a proposal or a description, summarize the resolved decisions so they can feed the spec.
