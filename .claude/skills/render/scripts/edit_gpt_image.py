...
# ///
"""
Ported from disler/planf3 (https://github.com/disler/planf3), MIT license.
Part of the optional /render skill; used only when uv and OPENAI_API_KEY exist.
...
Usage:
    uv run edit_gpt_image.py "edit instruction" output.png input.png [input2.png ...] [options]

Examples:
    uv run edit_gpt_image.py "Add a rainbow in the sky" edited.png photo.png
    uv run edit_gpt_image.py "Make a group photo" group.png p1.png p2.png p3.png
...
def backup_if_exists(output_path: str) -> None:
    """Copy an existing output file into a backup/ dir beside it before overwrite.
    ...
    unrecoverable. Backups live next to the output (typically under gitignored
    specs/html/), never in the working directory, so an edit leaves no repo
    noise behind.
    """
...
def main():
    ...
    if args.compression is not None and args.format not in ("jpeg", "webp"):
        parser.error("--compression only applies to jpeg or webp output")
